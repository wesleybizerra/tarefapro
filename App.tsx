    ...
    if (viewMode === 'AUTH') {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
                <div className="w-full max-w-md">
                    ...
                    <div className="bg-white rounded-[2.5rem] p-10 shadow-xl border border-slate-100">
                        ...
                        <form onSubmit={handleLogin} className="space-y-4">
                            ...
                            <button type="submit" disabled={loading} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
                ...
                {showTerms && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-200">
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row">
            ...
            <aside className={`w-64 bg-white border-r border-slate-200 flex flex-col p-8 fixed h-full transition-transform lg:static z-[100] ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                ...
                <nav className="space-y-1">
                    <NavItem icon={<LayoutDashboard size={18} />} label="Início" active={activeTab === 'dashboard'} onClick={() => { setActiveTab('dashboard'); setMobileMenuOpen(false); }} />
                    <NavItem icon={<Award size={18} />} label="Missões" active={activeTab === 'missions'} onClick={() => { setActiveTab('missions'); setMobileMenuOpen(false); }} />
                    <NavItem icon={<Wallet size={18} />} label="Carteira" active={activeTab === 'wallet'} onClick={() => { setActiveTab('wallet'); setMobileMenuOpen(false); }} />
                    <NavItem icon={<UserIcon size={18} />} label="Meu Perfil" active={activeTab === 'profile'} onClick={() => { setActiveTab('profile'); setMobileMenuOpen(false); }} />
                </nav>
            ...
            <main className="flex-1 p-4 lg:p-8">
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
                    <div>
                        ...
                        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Resumo de Atividades</p>
                    </div>
                    <div className="flex items-center gap-4 bg-white p-2 pr-4 rounded-3xl shadow-sm border border-slate-100">
                        <div className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 font-black">
                            CS
                        </div>
                        <div className="text-right">
                            <p className="text-[9px] font-black text-slate-400 uppercase">Saldo Disponível</p>
                            <p className="text-base font-black text-indigo-600 leading-tight">R$ {USER_DATA.balance.available.toFixed(2)}</p>
                        </div>
                    </div>
                </header>
                ...
                <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white flex flex-col md:flex-row items-center justify-between shadow-xl shadow-indigo-100 relative overflow-hidden group">
                    <div className="relative z-10 text-center md:text-left">
                        <Badge variant="premium">Proteção Ativa</Badge>
                        <h2 className="text-2xl font-bold mt-2">Sua conta está segura</h2>
                        <p className="text-indigo-100 text-sm mt-1 max-w-sm">Criptografia de ponta a ponta AES-256 e auditoria LGPD ativa em tempo real.</p>
                    </div>
                    <button className="mt-6 md:mt-0 bg-white text-indigo-600 px-8 py-4 rounded-2xl font-black text-sm shadow-lg hover:scale-105 transition-transform relative z-10">
                        VERIFICAR DADOS
                    </button>
                    {/* Decor */}
                    ...
                    <div className="absolute bottom-0 left-0 w-32 h-32 bg-white/5 rounded-full -ml-10 -mb-10 group-hover:scale-125 transition-transform duration-700"></div>
                </div>

                {/* Missions Section (Placeholder) */}
                <div className="mt-12">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold">Missões Disponíveis</h3>
                        <button className="text-indigo-600 text-xs font-bold uppercase tracking-widest hover:underline">Ver todas</button>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[1, 2].map(i => (
                            <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 flex items-center justify-between group hover:border-indigo-200 transition-colors">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                                        {/* Fixed typo in icon component name (changed Russian word to imported 'Smartphone' component) */}
                                        <Smartphone size={24} />
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-slate-800">Avaliação App {i}</h4>
                                        <p className="text-xs text-slate-400 font-medium">Recompensa: R$ 5,00</p>
                                    </div>
                                </div>
                                <button className="bg-slate-50 text-indigo-600 p-2 rounded-xl group-hover:bg-indigo-600 group-hover:text-white transition-all">
