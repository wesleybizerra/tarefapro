  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(data.details || data.error || 'Erro ao processar saque.');
      }
    } catch (e: any) {
      ...
      setTimeout(() => {
        setWithdrawStatus(current => current === 'SUCCESS' ? 'IDLE' : current);
      }, 5000);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-md bg-white rounded-[3.5rem] p-12 shadow-3xl border border-slate-100">
        <div className="text-center mb-10">
          ...
          <Badge variant="premium">Acesso Master</Badge>
        </div>
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-4">E-mail</label>
            <input type="email" placeholder="seuemail@hotmail.com" onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full px-8 py-5 bg-slate-50 border border-slate-100 rounded-3xl outline-none focus:bg-white transition-all font-bold" required />
          </div>
          <div className="space-y-1">
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row font-sans">
      ...
      <main className="flex-1 p-6 lg:p-16 overflow-y-auto pb-32">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-16">
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-slate-900 leading-none">{viewMode === 'ADMIN' ? `Olá, ${userData.name.split(' ')[0]}` : 'Seu Painel'}</h1>
        ...
        {viewMode === 'ADMIN' && activeTab === 'admin_overview' && (
          <div className="space-y-8 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <StatCard title="Comissão (10%)" value={`R$ ${platformStats.adminCommission.toFixed(2)}`} icon={<DollarSign />} variant="dark" trend="Líquido Asaas" />
            ...
            <div className="bg-slate-900 rounded-[3rem] p-12 text-white relative overflow-hidden border border-slate-800">
              <div className="relative z-10 max-w-lg">
                ...
                <p className="text-slate-400 text-lg mt-4 font-medium">Suas comissões são processadas em tempo real e protegidas pela API Asaas.</p>
        ...
        {viewMode === 'USER' && activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-indigo-600 rounded-[3.5rem] p-16 text-white text-center shadow-3xl relative overflow-hidden group">
              ...
              <p className="text-indigo-100 text-xl mb-10 max-w-md mx-auto">Execute tarefas para testar o fluxo de pagamento e comissão automática.</p>
          ...
          <div className="max-w-2xl mx-auto py-10 animate-fade-in">
            <div className="bg-white p-12 rounded-[3.5rem] shadow-3xl border border-slate-100">
              ...
              {withdrawStatus === 'ERROR' && (
                <div className="mb-6 p-6 bg-red-50 text-red-700 rounded-3xl border border-red-100 flex items-start gap-4">
                  <AlertCircle className="shrink-0 mt-1" />
                  <div>
                    <p className="font-black text-xs uppercase mb-1">Erro no Processamento</p>
                    <p className="text-sm font-medium">{errorMessage}</p>
                  </div>
                  <button onClick={() => setWithdrawStatus('IDLE')} className="ml-auto text-[10px] font-black uppercase bg-red-100 px-3 py-1 rounded-lg">Fechar</button>
                </div>
              )}
              ...
              <button
                ...
                className={`w-full py-8 text-white rounded-[2rem] font-black text-xl shadow-3xl transition-all flex items-center justify-center gap-3 ${withdrawStatus === 'PROCESSING' ? 'bg-slate-400' : 'bg-indigo-600 hover:bg-indigo-700 active:scale-95 disabled:opacity-30'}`}
              >
                {withdrawStatus === 'PROCESSING' ? <RefreshCw className="animate-spin" /> : <Zap fill="currentColor" size={24} />}
                {withdrawStatus === 'PROCESSING' ? 'COMUNICANDO BANCO...' : 'SOLICITAR PIX IMEDIATO'}
              </button>
              ...
              <p className="text-center text-[10px] font-black text-slate-400 uppercase tracking-widest mt-10 flex items-center justify-center gap-2">
                <ShieldCheck size={14} /> Transação Criptografada via Railway API
              </p>
            </div>
          </div>
        )}
